...
const App = () => {
  ...
  const cartTotal = useMemo(() => cart.reduce((acc, item) => acc + (item.price * item.cartQty), 0), [cart]);
  const troco = useMemo(() => {
  ...
  const finalizeSale = async () => {
    ...
    try {
      ...
      setCart([]); setAmountReceived(''); setIsCheckoutOpen(false);
  ...
  const handleAuth = async (e) => {
    ...
    } catch (error) { Swal.fire('Erro', 'Verifique os dados.', 'error'); }
  };
  ...
  const handleAddProduct = async (e) => {
    ...
    } catch (err) { Swal.fire('Erro', 'Erro ao salvar', 'error'); } finally { setIsSaving(false); }
  };

  const handleDeleteProduct = async (product) => {
    if (!isAdmin) return;
    const res = await Swal.fire({ title: 'Excluir?', icon: 'warning', showCancelButton: true });
  ...
  // --- ESTATÍSTICAS COMPLETAS (DASHBOARD E FECHAMENTO) ---
  const stats = useMemo(() => {
    ...
    const transList = [...transactions].sort((a, b) => (b.timestamp?.toMillis || 0) - (a.timestamp?.toMillis || 0));

    const transFiltradas = transList.filter(t => {
    ...
    const vendasFiltradas = transFiltradas.filter(t => t.type === 'venda');
    const resumoPagos = {
    ...
    const totalPeriodoVendas = vendasFiltradas.reduce((a, b) => a + (b.amount || 0), 0);
    const lucroPeriodo = vendasFiltradas.reduce((a, b) => a + (b.profit || 0), 0);
    const margem = totalPeriodoVendas > 0 ? (lucroPeriodo / totalPeriodoVendas) * 100 : 0;

    const rankingSemanaMap = {};
    vendasFiltradas.forEach(v => { rankingSemanaMap[v.productName] = (rankingSemanaMap[v.productName] || 0) + (v.quantity || 0); });
    const rankingSemana = Object.entries(rankingSemanaMap).sort(([,a],[,b])=>b-a).slice(0, 5);

    const vendasHora = Array.from({ length: 24 }, (_, i) => ({
      hora: `${i}:00`,
      total: vendasFiltradas.filter(v => {
    ...
    return { totalPeriodoVendas, lucroPeriodo, margem, resumoPagos, totalCaixaFinal: Object.values(resumoPagos).reduce((a,b)=>a+b, 0), rankingSemana, vendasHora, lowStock: products.filter(p => p.category === 'Mercadinho' && (p.stock || 0) <= (p.minStock || 0)) };
    ...
    return (
      <div className="min-h-screen bg-slate-100 flex items-center justify-center p-4">
        <div className="bg-white w-full max-w-md rounded-[3rem] shadow-2xl p-10 text-center border">
          ...
          <button onClick={handleGoogleLogin} className="w-full bg-white border-2 p-4 rounded-2xl font-black mt-4 text-xs uppercase">Google</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row font-sans">
      <aside className="w-full md:w-72 bg-slate-900 text-white p-8 flex flex-col shadow-2xl">
        <div className="flex items-center gap-3 mb-10 text-xl font-black text-emerald-400 uppercase tracking-tighter"><Utensils /> PRIMAVERA</div>
        <nav className="space-y-3 flex-1 text-xs font-black uppercase tracking-widest">
          {isAdmin && <button onClick={() => setActiveTab('dashboard')} className={`w-full text-left p-5 rounded-2xl flex items-center gap-4 transition-all ${activeTab === 'dashboard' ? 'bg-emerald-600 shadow-xl scale-105' : 'hover:bg-slate-800'}`}><LayoutDashboard size={20}/> Dashboard</button>}
          <button onClick={() => setActiveTab('cardapio')} className={`w-full text-left p-5 rounded-2xl flex items-center gap-4 transition-all ${activeTab === 'cardapio' ? 'bg-orange-500 shadow-xl scale-105' : 'hover:bg-slate-800'}`}><ShoppingCart size={20}/> Vendas / PDV</button>
          <button onClick={() => setActiveTab('estoque')} className={`w-full text-left p-5 rounded-2xl flex items-center gap-4 transition-all ${activeTab === 'estoque' ? 'bg-blue-600 shadow-xl scale-105' : 'hover:bg-slate-800'}`}><Package size={20}/> Estoque</button>
          {isAdmin && <button onClick={() => setActiveTab('fechamento')} className={`w-full text-left p-5 rounded-2xl flex items-center gap-4 transition-all ${activeTab === 'fechamento' ? 'bg-purple-600 shadow-xl scale-105' : 'hover:bg-slate-800'}`}><Calculator size={20}/> Fechamento</button>}
        </nav>
        <button onClick={() => signOut(auth)} className="flex items-center gap-3 text-red-400 font-black uppercase text-[10px]"><LogOut size={16}/> Sair</button>
      </aside>
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        <header className="flex justify-between items-center mb-10">
          ...
          <div className="flex gap-4">
            <button onClick={() => setIsSangriaModalOpen(true)} className="bg-red-100 text-red-600 px-6 py-4 rounded-2xl font-black text-xs uppercase flex items-center gap-2 hover:bg-red-200 transition-all"><MinusCircle size={18}/> Sangria</button>
          ...
          <div className="space-y-8 animate-in fade-in duration-700">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div className="bg-slate-900 p-8 rounded-[2.5rem] text-white shadow-2xl col-span-1 md:col-span-2 flex flex-col justify-center">
                    <p className="text-emerald-400 text-xs font-black uppercase tracking-widest mb-2">Faturamento {periodoFechamento}</p>
                    <h3 className="text-6xl font-black tracking-tighter mb-4">R$ {stats.totalPeriodoVendas.toFixed(2)}</h3>
                    <div className="flex gap-4">
                        <div className="bg-white/10 p-3 rounded-xl flex-1"><p className="text-[10px] font-bold text-slate-400 uppercase">Lucro Líquido</p><p className="text-xl font-black text-emerald-400">R$ {stats.lucroPeriodo.toFixed(2)}</p></div>
                        <div className="bg-white/10 p-3 rounded-xl flex-1"><p className="text-[10px] font-bold text-slate-400 uppercase">Margem</p><p className="text-xl font-black text-blue-400">{stats.margem.toFixed(1)}%</p></div>
                    </div>
                </div>
                <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-200 text-center flex flex-col justify-center">
                    <p className="text-slate-400 text-[10px] font-black uppercase mb-2">Ticket Médio</p>
                    <h3 className="text-3xl font-black text-slate-800">R$ {(stats.totalPeriodoVendas / (transactions.filter(t=>t.type==='venda').length || 1)).toFixed(2)}</h3>
                </div>
                <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-200 text-center flex flex-col justify-center">
                    <p className="text-slate-400 text-[10px] font-black uppercase mb-2">Críticos</p>
                    <h3 className="text-3xl font-black text-orange-500">{stats.lowStock.length} Itens</h3>
                </div>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-white p-8 rounded-[3rem] shadow-sm border"><h3 className="font-black text-slate-800 uppercase text-xs mb-8 flex items-center gap-2"><TrendingUp size={16} className="text-blue-500"/> Fluxo de Vendas</h3><div className="h-64"><ResponsiveContainer><AreaChart data={stats.vendasHora}><XAxis dataKey="hora" axisLine={false} tickLine={false} tick={{fontSize: 10}} dy={10}/><Tooltip contentStyle={{borderRadius: '20px', border: 'none'}}/><Area type="monotone" dataKey="total" stroke="#10b981" strokeWidth={4} fill="#d1fae5"/></AreaChart></ResponsiveContainer></div></div>
                <div className="bg-white p-8 rounded-[3rem] shadow-sm border"><h3 className="font-black text-slate-800 uppercase text-xs mb-6">Top 5 Vendidos</h3><div className="space-y-4">{stats.rankingSemana.map(([name, qty], i) => (<div key={name} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl"><span className="font-black text-slate-700 text-xs uppercase">{i+1}. {name}</span><span className="bg-emerald-100 text-emerald-600 px-3 py-1 rounded-lg font-black text-xs">{qty} un</span></div>))}</div></div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'cardapio' && (
          <div className="flex flex-col lg:flex-row gap-8 animate-in fade-in duration-500 font-black">
            <div className="flex-1 space-y-6">
                ...
                {saleSearch && (
                  <div className="absolute z-50 w-full mt-4 bg-white border-2 border-slate-100 rounded-[2.5rem] shadow-2xl max-h-96 overflow-y-auto p-4 space-y-2">
                    {products.filter(p => p.name.toLowerCase().includes(saleSearch.toLowerCase()) || p.barcode?.includes(saleSearch)).map(p => (
                      <button key={p.id} onClick={() => addToCart(p)} className="w-full flex justify-between items-center p-5 hover:bg-emerald-50 rounded-2xl transition-all group border-b border-slate-50">
                        <div className="text-left"><p className="font-black text-slate-800 uppercase text-sm">{p.name}</p><p className="text-[10px] text-slate-400 uppercase">Estoque: {p.stock}</p></div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {products.filter(p => p.category !== 'Mercadinho').slice(0, 8).map(p => (
                   <button key={p.id} onClick={() => addToCart(p)} className="bg-white p-6 rounded-[2.5rem] shadow-sm hover:shadow-md transition-all text-center border border-slate-100 active:scale-95">
            ...
            <div className="w-full lg:w-96 bg-white rounded-[3rem] shadow-2xl flex flex-col h-[calc(100vh-200px)] sticky top-8 border border-slate-100 overflow-hidden">
              <div className="p-8 border-b flex justify-between items-center bg-slate-50">
                <h3 className="uppercase tracking-widest text-sm flex items-center gap-2"><Receipt size={18}/> Cupom</h3>
                <span className="bg-emerald-600 text-white px-3 py-1 rounded-full text-[10px] font-black">{cart.length} ITENS</span>
              </div>
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                {cart.length === 0 ? <p className="text-center text-slate-300 mt-20 uppercase text-xs font-black">Caixa Livre</p> : cart.map(item => (
                  <div key={item.id} className="flex justify-between items-center group font-black">
                    <div className="flex-1">
                      <p className="text-xs uppercase truncate w-32">{item.name}</p>
                      <div className="flex items-center gap-2 mt-1">
                          <button onClick={()=>updateCartQty(item.id, item.cartQty - 1)} className="w-6 h-6 bg-slate-100 rounded-full">-</button>
                          <span className="text-xs">{item.cartQty}</span>
                          <button onClick={()=>updateCartQty(item.id, item.cartQty + 1)} className="w-6 h-6 bg-slate-100 rounded-full">+</button>
                      </div>
                    </div>
                    <div className="text-right"><p className="text-sm">R$ {(item.price * item.cartQty).toFixed(2)}</p></div>
                  </div>
                ))}
              </div>
              <div className="p-8 bg-slate-900 text-white">
                <div className="flex justify-between items-end mb-6 font-black">
                  <p className="uppercase text-xs text-slate-400">Total</p>
                  <h4 className="text-5xl text-emerald-400">R$ {cartTotal.toFixed(2)}</h4>
                </div>
                <button disabled={cart.length === 0} onClick={() => setIsCheckoutOpen(true)} className="w-full bg-emerald-600 py-5 rounded-2xl uppercase text-xs shadow-xl hover:bg-emerald-500 transition-all disabled:opacity-50 font-black">Finalizar Venda</button>
              </div>
            </div>
          </div>
        )}

        {/* --- FECHAMENTO (RESTURADO COM TODOS FILTROS) --- */}
        {activeTab === 'fechamento' && isAdmin && (
          <div className="space-y-8 animate-in fade-in">
            <div className="flex flex-wrap gap-4 items-center">
                <div className="flex bg-white p-2 rounded-2xl border w-fit shadow-sm font-black">
                ...
                {periodoFechamento === 'personalizado' && (
                    <div className="flex gap-2 items-center">
                        <input type="date" className="p-3 bg-white border rounded-xl font-bold text-xs" value={dataInicio} onChange={(e)=>setDataInicio(e.target.value)} />
                        <span className="text-slate-400 font-black">ATÉ</span>
                        <input type="date" className="p-3 bg-white border rounded-xl font-bold text-xs" value={dataFim} onChange={(e)=>setDataFim(e.target.value)} />
                    </div>
                )}
                <button onClick={exportarPlanilha} className="bg-slate-900 text-white px-6 py-4 rounded-2xl font-black text-[10px] uppercase flex items-center gap-2 ml-auto shadow-xl"><Download size={16}/> Baixar CSV</button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              {['Dinheiro', 'PIX', 'Cartão', 'Vale', 'Sangria'].map(label => (
                <div key={label} className="bg-white p-8 rounded-[2.5rem] border shadow-sm text-center font-black">
                  <p className="text-[10px] font-black text-slate-400 uppercase mb-2">{label}</p>
                  <p className={`text-2xl tracking-tight ${label === 'Sangria' ? 'text-red-500' : 'text-slate-800'}`}>R$ {Math.abs(stats.resumoPagos[label === 'Cartão' ? 'Cartão' : label === 'Vale' ? 'Vale' : label] || 0).toFixed(2)}</p>
                </div>
              ))}
            </div>
            <div className="bg-emerald-600 p-16 rounded-[4rem] text-white shadow-2xl flex justify-between items-center font-black">
                <div><p className="uppercase text-xs opacity-80">Saldo Total em Caixa ({periodoFechamento})</p><h3 className="text-8xl tracking-tighter">R$ {stats.totalCaixaFinal.toFixed(2)}</h3></div>
        ...
        {activeTab === 'estoque' && (
          <div className="bg-white rounded-[3rem] border shadow-sm overflow-hidden font-black">
            <div className="p-8 bg-slate-50 border-b flex justify-between items-center uppercase">
                <div className="flex gap-4 items-center flex-1">
                    <Search className="text-slate-400"/><input className="bg-transparent outline-none w-full text-sm font-black" placeholder="Buscar no estoque..." value={searchTerm} onChange={(e)=>setSearchTerm(e.target.value)}/>
                </div>
                <button onClick={enviarListaComprasZap} className="bg-emerald-600 text-white px-6 py-3 rounded-2xl text-[10px] uppercase">WhatsApp Compras</button>
            </div>
            <table className="w-full text-left">
              <thead className="bg-slate-50 text-[10px] uppercase text-slate-400"><tr><th className="p-8">Item</th><th className="p-8">Saldo</th><th className="p-8 text-center">Ações</th></tr></thead>
              <tbody className="divide-y">
                {products.filter(p => p.name.toLowerCase().includes(searchTerm.toLowerCase())).map(p => (
                  <tr key={p.id} className="hover:bg-slate-50 transition-all font-black">
                    <td className="p-8 text-xs uppercase">{p.name}</td>
                    <td className="p-8"><span className={`px-4 py-2 rounded-xl text-[11px] ${p.stock <= (p.minStock || 0) ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-600'}`}>{p.stock} un</span></td>
                    <td className="p-8 flex justify-center gap-4">
                      {isAdmin && <button onClick={() => { setEditingProduct(p); setIsProductModalOpen(true); }} className="text-blue-500"><Edit3 size={18}/></button>}
                      {isAdmin && <button onClick={() => handleDeleteProduct(p)} className="text-red-500"><Trash2 size={18}/></button>}
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {/* --- MODAL PRODUTO (CORRETO E SEGURO) --- */}
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-[3rem] w-full max-w-md p-12 shadow-2xl relative font-black animate-in zoom-in">
            ...
            <form onSubmit={handleAddProduct} className="space-y-6 text-left">
              <input name="name" placeholder="Nome do Produto" defaultValue={editingProduct?.name || ''} required className="w-full p-5 bg-slate-50 rounded-2xl outline-none border-2 border-transparent focus:border-emerald-500 font-black" />
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <select className="w-full p-5 bg-slate-900 text-white rounded-2xl outline-none appearance-none text-center" value={selectedMethod} onChange={(e) => setSelectedMethod(e.target.value)}>
                  ...
                  <option value="Alimentação">🍴 Vale</option>
                </select>
              ...
              {selectedMethod === 'Dinheiro' && troco > 0 && (
                <div className="bg-emerald-50 p-6 rounded-2xl flex justify-between items-center border border-emerald-100">
                  <p className="text-emerald-700 text-xs uppercase">Troco</p><p className="text-4xl text-emerald-600">R$ {troco.toFixed(2)}</p>
                </div>
              )}
              <button onClick={finalizeSale} className="w-full bg-emerald-600 text-white py-6 rounded-[2rem] uppercase shadow-xl hover:bg-emerald-700 active:scale-95 font-black">Concluir Venda</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isSangriaModalOpen && (
        <div className="fixed inset-0 bg-red-900/40 backdrop-blur-md flex items-center justify-center p-4 z-50 font-black">
          <div className="bg-white rounded-[3rem] w-full max-w-md p-12 shadow-2xl relative">
            ...
            <h3 className="text-3xl mb-10 text-red-600 uppercase flex items-center gap-3">Sangria</h3>
